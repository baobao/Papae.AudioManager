...
namespace Papae.UnitySDK.Managers
{
    ...
    public class AudioManager : MonoBehaviour
    {
        ...
        [Header("Background Music Properties")] [Tooltip("Is the background music mute")] [SerializeField]
        bool _musicOn = true;

        [Tooltip("The background music volume")] [Range(0, 1)] [SerializeField]
        float _musicVolume = 1f;

        [Tooltip("Use the current music volume settings on initialisation start")] [SerializeField]
        bool _useMusicVolOnStart = false;

        [Tooltip(
            "The target group for the background music to route its their signals. If none is to be used, then leave unassigned or blank")]
        [SerializeField]
        AudioMixerGroup _musicMixerGroup = null;

        [Tooltip("The exposed volume parameter name of the music mixer group")] [SerializeField]
        string _volumeOfMusicMixer = string.Empty;

        [Space(3)] [Header("Sound Effect Properties")] [Tooltip("The sound effects volume")] [SerializeField]
        bool _soundFxOn = true;

        [Tooltip("The sound effects volume")] [Range(0, 1)] [SerializeField]
        float _soundFxVolume = 1f;

        [Tooltip("Use the current sound effect volume settings on initialisation start")] [SerializeField]
        bool _useSfxVolOnStart = false;

        [Tooltip(
            "The target group for the sound effects to route its their signals. If none is to be used, then leave unassigned or blank")]
        [SerializeField]
        AudioMixerGroup _soundFxMixerGroup = null;

        [Tooltip("The exposed volume parameter name of the sound effects mixer group")] [SerializeField]
        string _volumeOfSFXMixer = string.Empty;

        [Space(3)] [Tooltip("A list of all audio clips attached to the AudioManager")] [SerializeField]
        ...
        {
            return !soundEffect.Source.isPlaying && FloatEquals(soundEffect.PlaybackPosition, 0) &&
                   soundEffect.Time <= 0.09f;
        }
        ...
        private bool IsMusicAltered()
        {
            ...
            bool flag = musicOn != _musicOn || musicOn != !musicSource.mute ||
                        !FloatEquals(currentMusicVol, _musicVolume);
        ...
        private void PlayMusicFromSource(ref AudioSource audio_source, AudioClip clip, float playback_position,
            float pitch)
        {
            try
            {
            ...
            float pitch, float playback_position = 0)
        {
            ...
            else
            {
                ...
                {
                    Debug.LogWarning(
                        "Trying to perform a transition on the background music while one is still active");
                    return;
                }
        ...
        public AudioSource PlaySFX(AudioClip clip, Vector2 location, float duration, float volume,
        ...
        /// <param name="singleton">Is the sound effect a singleton.</param>
        /// <param name="callback">Action callback to be invoked after the sound has finished.</param>
        public AudioSource PlaySFX(AudioClip clip, Vector2 location, float duration, bool singleton = false,
        ...
        /// <param name="pitch">Pitch level of the clip.</param>
        /// <param name="callback">Action callback to be invoked after the sound has finished.</param>
        public AudioSource RepeatSFX(AudioClip clip, Vector2 location, int repeat, float volume, bool singleton = false,
            float pitch = 1f, Action callback = null)
        {
            if (clip == null)
            {
                return null;
            ...
            if (repeat != 0)
            {
                ...
                if (index >= 0)
                {
                    // Reset the duration if it exists in the pool
                    SoundEffect singletonSFx = sfxPool[index];
                    singletonSFx.Duration =
                        singletonSFx.Time = repeat > 0 ? clip.length * repeat : float.PositiveInfinity;
                    sfxPool[index] = singletonSFx;
        ...
        /// <param name="singleton">Is the sound effect a singleton.</param>
        /// <param name="callback">Action callback to be invoked after the sound has finished.</param>
        public AudioSource RepeatSFX(AudioClip clip, Vector2 location, int repeat, bool singleton = false,
            Action callback = null)
        {
        ...
        public AudioClip LoadClip(string path, bool add_to_playlist = false)
        {
            ...
            if (clip == null)
            {
                Debug.LogError(string.Format("AudioClip '{0}' not found at location {1}", path,
                    System.IO.Path.Combine(Application.dataPath, "/Resources/" + path)));
                return null;
            }
        ...
        IEnumerator LoadAudioClipFromUrl(string audio_url, AudioType audio_type, Action<AudioClip> callback)
        {
            using (UnityEngine.Networking.UnityWebRequest www =
                UnityEngine.Networking.UnityWebRequestMultimedia.GetAudioClip(audio_url, audio_type))
            {
